...
const updateSnapIndicator = (worldPoint: THREE.Vector3 | null, type: 'VERTEX' | 'EDGE' | 'SECTION' = 'VERTEX') => {
    ...
    if ((window as any).debugSphere) {
        (window as any).debugSphere.visible = false;
    }
    if (typeof snappingCursor !== 'undefined' && snappingCursor) {
...
const applyGlobalSnap = (intersects: THREE.Intersection[], mousePixelPos?: { x: number; y: number }) => {
    ...
    const closest = intersects.find(i => {
        ...
        if (i.object === (window as any).debugSphere || i.object === snappingCursor || i.object.name === 'debugSphere' || (i.object as any).userData?.isHelper) return false;
        return (i.object instanceof THREE.Mesh || i.object instanceof THREE.InstancedMesh);
    });
    ...
    try {
        ...
        const getVertexWorld = (idx: number) => {
            ...
            if (idx >= 0 && idx < pos.count) {
                ...
                if (closest.object instanceof THREE.InstancedMesh && closest.instanceId !== undefined) {
                    const instanceMatrix = new THREE.Matrix4();
                    closest.object.getMatrixAt(closest.instanceId, instanceMatrix);
                    v.applyMatrix4(instanceMatrix);
                }
        ...
        const va = getVertexWorld(closest.face.a);
        const vb = getVertexWorld(closest.face.b);
        const vc = getVertexWorld(closest.face.c);

        const candidatePool: THREE.Vector3[] = [va, vb, vc];
        ...
        const sectionCandidates = getSectionSnapCandidates([va, vb, vc], closest.point);
...
const setupDebugSphere = (scene: THREE.Scene) => {
    ...
    debugSphere.raycast = () => {};
    (debugSphere as any).userData = { isHelper: true };
...
highlighter.setup({
    ...
    select: {
        name: 'select',
        material: new THREE.MeshBasicMaterial({ color: 0xff0000, depthTest: false, opacity: 0.8, transparent: true })
    },
    hover: {
        name: 'hover',
...
const listDriveModels = async (): Promise<Array<{ name: string; fragId: string; jsonId?: string | null }>> => {
    ...
    const prj = currentParams.get('project') || '';
    const dfn = currentParams.get('driveFolderName') || '';
    if (prj) url.searchParams.set('project', prj);
    if (dfn) url.searchParams.set('driveFolderName', dfn);
...
function extractAllElementsGlobal() {
    ...
    for (const [modelUUID, model] of loadedModels.entries()) {
        ...
        for (const expressID of allIds) {
            ...
            const searchStr = [classification, name, material].join(' ').toLowerCase();
            const diameter = '';
            ...
            const isPipe = (searchStr.includes('pipe') || searchStr.includes('conduit') || searchStr.includes('cablecarrier') || searchStr.includes('tuber') || searchStr.includes('tubo') || searchStr.includes('canalizacion') || searchStr.includes('canalización') || searchStr.includes('coraza') || searchStr.includes('ducto') || searchStr.includes('bandeja')) && !isUnion;
...
function initProjectionToggle() {
    ...
    btn.addEventListener('click', async () => {
        ...
        try {
            await projectionApi.set(next);
        } catch (e) {
            console.warn("Projection set warning:", e);
        }
        ...
        // Wait a short moment to allow transition to finish before allowing another click
        setTimeout(() => {
    ...
    function updateBulkActionBar() {
        ...
        bulkBar.innerHTML = `
            ...
            <div class="q-bulk-bar-right">
                ...
                <button type="button" class="q-btn-xs" data-status="COMPRADO" style="background: #333333; color: white;">Comprado</button>
...
function buildBIMContextForAI() {
    ...
    const matrixByLevelAndMaterial: Record<string, Record<string, { count: number; area: number; volume: number }>> = {};
    const matrixByLevelAndCategory: Record<string, Record<string, { count: number; area: number; volume: number }>> = {};

    for (const el of elements) {
        const catKey = el.category || el.name || 'Sin Categoría';
        const lvlKey = el.level || 'SIN NIVEL';
        const matKey = el.material || 'SIN MATERIAL';
        const vol = el.volume || 0;
        const area = el.area || 0;

        if (!categories[catKey]) categories[catKey] = { count: 0, area: 0, volume: 0 };
        categories[catKey].count++;
        categories[catKey].area += area;
        categories[catKey].volume += vol;

        if (lvlKey !== 'SIN NIVEL') {
            if (!levels[lvlKey]) levels[lvlKey] = { count: 0, area: 0, volume: 0 };
            levels[lvlKey].count++;
            levels[lvlKey].area += area;
            levels[lvlKey].volume += vol;
        }

        if (matKey !== 'SIN MATERIAL') {
            if (!materials[matKey]) materials[matKey] = { count: 0, area: 0, volume: 0 };
            materials[matKey].count++;
            materials[matKey].area += area;
            materials[matKey].volume += vol;
        }

        if (el.pilote) {
            if (!pilotes[el.pilote]) pilotes[el.pilote] = { count: 0, area: 0, volume: 0 };
            pilotes[el.pilote].count++;
            pilotes[el.pilote].area += area;
            pilotes[el.pilote].volume += vol;
        }

        // Matrix level x material
        if (lvlKey !== 'SIN NIVEL' && matKey !== 'SIN MATERIAL') {
            if (!matrixByLevelAndMaterial[lvlKey]) matrixByLevelAndMaterial[lvlKey] = {};
            if (!matrixByLevelAndMaterial[lvlKey][matKey]) matrixByLevelAndMaterial[lvlKey][matKey] = { count: 0, area: 0, volume: 0 };
            matrixByLevelAndMaterial[lvlKey][matKey].count++;
            matrixByLevelAndMaterial[lvlKey][matKey].area += area;
            matrixByLevelAndMaterial[lvlKey][matKey].volume += vol;
        }

        // Matrix level x category
        if (lvlKey !== 'SIN NIVEL') {
            if (!matrixByLevelAndCategory[lvlKey]) matrixByLevelAndCategory[lvlKey] = {};
            if (!matrixByLevelAndCategory[lvlKey][catKey]) matrixByLevelAndCategory[lvlKey][catKey] = { count: 0, area: 0, volume: 0 };
            matrixByLevelAndCategory[lvlKey][catKey].count++;
            matrixByLevelAndCategory[lvlKey][catKey].area += area;
            matrixByLevelAndCategory[lvlKey][catKey].volume += vol;
        }
    }

    const roundStats = (obj: any) => {
        for (const k in obj) {
            if (typeof obj[k] === 'object' && obj[k] !== null) {
                if ('area' in obj[k]) {
                    obj[k].area = Math.round(obj[k].area * 100) / 100;
                    obj[k].volume = Math.round(obj[k].volume * 100) / 100;
                } else {
                    roundStats(obj[k]);
                }
            }
    ...
    roundStats(matrixByLevelAndMaterial);
    roundStats(matrixByLevelAndCategory);
    ...
    return {
        ...
        materials,
        pilotes,
        matrixByLevelAndMaterial,
        matrixByLevelAndCategory
    };
}
...
function setupMeasurementTools() {
    ...
    if (!snappingCursor) {
        ...
        snappingCursor.raycast = () => {}; // Never raycast helper cursor
        (snappingCursor as any).userData = { isHelper: true };
...
function getStoredUserAccount() {
    if (window.location.search.includes('bypassAuth=true')) {
        return { name: 'Model Testing Agent', email: 'agent@nora.com', role: 'Tester' };
    }
