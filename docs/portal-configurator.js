document.addEventListener('DOMContentLoaded', async () => {
  let CONFIG_URL = 'portal-config.json';
  ...
  if (empresaId) {
    try {
      const res = await fetch(`empresas.json?t=${Date.now()}`);
      if (res.ok) {
        const empresas = await res.json();
        CURRENT_EMPRESA = empresas.find(e => e.id === empresaId);
        if (CURRENT_EMPRESA && CURRENT_EMPRESA.configUrl) {
          CONFIG_URL = CURRENT_EMPRESA.configUrl;
        }
      }
  ...
  const GreenI_TEMPLATE = {
    ...
    logoUrl: 'https://i.postimg.cc/P5rsf8Yb/Green I.png',
  ...
  const DEFAULT_CONFIG = {
    portal: {
      ...
      logoLight: 'https://i.postimg.cc/L4r0gSvV/LOGO-TEXTO-NORA-NEGRO.png',
      logoDark: 'https://i.postimg.cc/FFfBKzb8/LOGO-TEXTO-NORA-BLANCO.png',
      footerLogoLight: 'https://i.postimg.cc/tR3YSryT/LOGO-NORA-NEGRO.png',
      footerLogoDark: 'https://i.postimg.cc/SQ6JTZqj/LOGO-NORA-BLANCO.png',
      footerLogoSecondaryLight: 'https://i.postimg.cc/tR3YSryT/LOGO-NORA-NEGRO.png',
      footerLogoSecondaryDark: 'https://i.postimg.cc/SQ6JTZqj/LOGO-NORA-BLANCO.png',
    },
    projects: [],
    publicaciones: [],
    solicitudes: [],
  ...
  const DEFAULT_PROJECT = {
    ...
    landing: {
      ...
      address: '',
      logoUrl: 'https://i.postimg.cc/W3trgjZX/FAVICON-NORA-NEGRO.png',
      portalLogoLight: 'https://i.postimg.cc/L4r0gSvV/LOGO-TEXTO-NORA-NEGRO.png',
      portalLogoDark: 'https://i.postimg.cc/FFfBKzb8/LOGO-TEXTO-NORA-BLANCO.png',
      viewerLogoLight: 'https://i.postimg.cc/L4r0gSvV/LOGO-TEXTO-NORA-NEGRO.png',
      viewerLogoDark: 'https://i.postimg.cc/FFfBKzb8/LOGO-TEXTO-NORA-BLANCO.png',
  ...
  const el = {
    ...
    projectEmpresa: document.getElementById('project-empresa'),
    ...
    publishGithubBtn: document.getElementById('publish-github-btn'),
  ...
  // Update UI with current empresa context if any
  if (CURRENT_EMPRESA) {
    const contextEl = document.createElement('div');
    contextEl.className = 'bg-primary text-white text-xs px-3 py-1 rounded-full font-semibold uppercase tracking-wider ml-4';
    contextEl.textContent = `Editando: ${CURRENT_EMPRESA.name}`;
    document.querySelector('header .flex.items-center').appendChild(contextEl);
  }
  ...
  function normalizeProject(project) {
    ...
    return {
      ...
      empresaId: String(project?.empresaId || '').trim(),
  ...
  function fillProjectForm() {
    ...
    el.projectEmpresa.value = project.empresaId || '';
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.warn(`Config file ${CONFIG_URL} not found, initializing with defaults.`);
          return normalizeConfig(clone(DEFAULT_CONFIG));
        }
        throw new Error(`No se pudo leer ${CONFIG_URL}`);
      }
  ...
  async function pushToGitHub(content) {
    ...
    if (!token) {
      ...
      if (!token) {
        showBanner('Se requiere el token de GitHub para publicar en la nube.', 'error');
        return;
      }
    ...
    const branch = 'main';
    const files = [CONFIG_URL, `docs/${CONFIG_URL}`];
    
    showBanner(`Publicando configuración de ${CURRENT_EMPRESA ? CURRENT_EMPRESA.name : 'la empresa'} en la nube... Por favor espera.`, 'info');
    ...
    try {
      for (const path of files) {
        ...
        try {
          ...
          console.warn('File not found, creating new one.', e);
        }

        const body = {
          message: 'Update portal configuration from CDE Editor',
      ...
      showBanner('Configuración publicada exitosamente en GitHub. Los cambios se verán reflejados en un minuto.', 'success');
    } catch (err) {
  ...
  function bindProjectInputs() {
    ...
    [
      ...
      [el.projectEmpresa, (project, value) => { project.empresaId = value; }],
  ...
  function applyGreenIBase() {
    updateCurrentProject((project) => {
      project.landing = {
        ...project.landing,
        ...clone(GreenI_TEMPLATE),
        title: project.name ? project.name.toUpperCase() : GreenI_TEMPLATE.title,
        cityLabel: project.city ? project.city.toUpperCase() : GreenI_TEMPLATE.cityLabel,
      };
      project.modules = {
    ...
    fillProjectForm();
    showBanner('Se aplicó la base visual de Green I al proyecto seleccionado.', 'success');
  }

  function attachButtons() {
    el.applyGreenIBtn.addEventListener('click', applyGreenIBase);
    ...
    if (el.publishGithubBtn) {
      el.publishGithubBtn.addEventListener('click', () => {
        const jsonString = JSON.stringify(workingConfig, null, 2) + '\n';
