    ...
    <div className="w-96 border-l border-[#C5C0C8] bg-white flex flex-col h-full shadow-xl">
      <div className="p-4 border-b border-[#C5C0C8]/30 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-bold text-[#000000] flex items-center gap-2">
            <i className="fa-solid fa-wand-magic-sparkles text-[#171717]"></i>
            Asistente AI
          </h2>
          {onClose && (
            <button onClick={onClose} className="text-[#A49FA6] hover:text-[#171717] sm:hidden transition-colors">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-[#C5C0C8]/20 p-1 rounded-lg">
            <button 
              ...
              className={`px-3 py-1 text-sm rounded-md transition focus:outline-none focus:ring-2 focus:ring-[#171717] ${activeTab === 'chat' ? 'bg-white shadow-sm text-[#171717] font-medium' : 'text-[#827E84] hover:text-[#605E62]'}`}
            >
              Chat
            </button>
            <button 
              onClick={() => setActiveTab('summary')}
              className={`px-3 py-1 text-sm rounded-md transition focus:outline-none focus:ring-2 focus:ring-[#171717] ${activeTab === 'summary' ? 'bg-white shadow-sm text-[#171717] font-medium' : 'text-[#827E84] hover:text-[#605E62]'}`}
            >
              Resumen
            </button>
          </div>
          {onClose && (
            <button onClick={onClose} className="ml-2 text-[#A49FA6] hover:text-[#171717] hidden sm:block transition-colors focus:outline-none focus:ring-2 focus:ring-[#171717] rounded">
              <i className="fa-solid fa-times"></i>
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-white" ref={scrollRef}>
        {!isPdfLoaded ? (
          ...
          <>
            {messages.length === 0 && (
              <div className="bg-[#171717]/5 border border-[#171717]/20 rounded-xl p-4 text-sm text-[#171717]">
            ...
            {!summary ? (
              <div className="flex flex-col items-center justify-center py-10">
                <button 
                  ...
                  className="bg-[#171717] text-white px-6 py-2 rounded-full text-sm font-medium hover:bg-[#171717]/90 transition flex items-center gap-2"
                >
              ...
              <div className="bg-white border border-[#C5C0C8] rounded-xl p-4 text-sm text-[#605E62] leading-relaxed shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <span className="text-xs font-bold uppercase tracking-wider text-[#171717]">Resumen Ejecutivo</span>
                  <button onClick={() => setSummary(null)} className="text-[#A49FA6] hover:text-[#171717]">
        ...
        <div className="p-4 border-t border-[#C5C0C8]/30">
          <form onSubmit={handleSendMessage} className="relative">
            <input 
              ...
              className="w-full pl-4 pr-12 py-3 bg-[#C5C0C8]/20 border-none rounded-xl text-sm focus:ring-2 focus:ring-[#171717] outline-none disabled:opacity-50 transition-all text-[#000000]"
            />
            <button 
              type="submit"
              disabled={!isPdfLoaded || isLoading || !input.trim()}
              className="absolute right-2 top-1.5 w-9 h-9 bg-[#171717] text-white rounded-lg flex items-center justify-center hover:bg-[#171717]/90 disabled:opacity-50 transition-colors focus:outline-none focus:ring-2 focus:ring-[#171717] focus:ring-offset-1"
            >
